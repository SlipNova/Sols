...
bool witness(u64 a, u64 n) {
    ...
    u64 x = mod_pow(a, d, n);
    if (x == 1 || x == n - 1)
        return false; // if we start at 1, -1 then a cannot be witness
    for (int i = 1; i < u; i++) {
        x = (u128)x * x % n;
        if (x == n - 1)
            return false; // x0 != 1 or -1, now if we hit -1 then next must be 1
                          // so a cannot be a witness cause rest of the sequence
                          // is juts 1
        if (x == 1)
            return true; // if we hit 1 means we didn't hit -1 before so n is
                         // composite, there are non trivial solutions to x^2 =
                         // 1, as we hit 1 before hitting -1 previously, so a is
                         // a witness
    }
    return true;
}
